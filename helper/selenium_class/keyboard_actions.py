...
from helper.selenium_class.constants import keys_selenium
...
class KeyboardActions:
    ...
    def _get_special_keys(self, value):
        """
        *Get and validate keys*

        - `@param value: string with value to keys module of selenium`
        - `@return: value from keys module`
        """

        res = None
        for key in keys_selenium.keys():
            if key == value:
                res = keys_selenium[key]
                break
        else:
            assert False, "Value is not found in keys module of selenium"
