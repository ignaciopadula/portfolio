...
def execution_selenium(context, scenario):
    if os.getenv('EXECUTION_TYPE') == "localhost":
        ...
        if "Mobile" in scenario.tags:
            context.browser.set_window_size(390, 844)
        else:
...
def config_driver_local(context,):
    ...
    try:
        if os.getenv('BROWSER') == "chrome":
            ...
            #chrome_options.add_argument("user-data-dir=/home/ipadula/.config/google-chrome/SeleniumProfile")
            if os.getenv('HEADLESS') == "true":
                chrome_options.add_argument("--headless=new")  # HEADLESS moderno (Chrome 109+)
            ...
            service = ChromeService(executable_path=rute_driver)
...
class SeleniumPlugin:
    ...
    def before_scenario(self, context, scenario):
        ...
        if os.getenv('RELOADBROWSER') == "true" and os.getenv('EXECUTION_TYPE') == "localhost":
            execution_selenium(context, scenario)
